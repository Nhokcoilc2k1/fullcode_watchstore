...
function UpdateCategory({editCategory, setEditCategory, render}) {
    ...
    const {values ,setValues, errors, handleChange,handleSubmit, handleBlur, touched} = useFormik({
        ...
        onSubmit : async() => {
            ...
            if(data.image) formData.append('image', typeof data.image === 'string' ? preview.image : data.image[0])
        ...
        <div className={cx('wrapper', 'update-page')}>
            <div className={cx('inner')}>
                <div className={cx('box-header')}>
                    ...
                    <Button onClick={() => setEditCategory(null)} className={cx('header-btn')}>Quay lại</Button>
                </div>
